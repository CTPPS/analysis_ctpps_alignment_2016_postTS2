...
int main()
{
	...
	for (const auto &cfg : cfgs)
	{
		...
		for (const auto &fill : fills)
		{
			...
			string ref_x_sh = "data_alig-version2_fill_5322_" + cfg + "_DS1";

			TFile *f_in_x_sh_alt = TFile::Open((dir + "/match.root").c_str());
			...
			for (const auto &rp : rps)
			{
				...
				if (!sector56)
				{
					...
					if (f_in_x_sh_alt)
					{
						TGraph *g_results = (TGraph *) f_in_x_sh_alt->Get((ref_x_sh + "/" + rp + "/method x/g_results").c_str());

						//printf("* %s, %p\n", rp.c_str(), g_results);

						if (g_results)
						{
							const double x_sh = g_results->GetY()[0];
							const double x_sh_unc = g_results->GetY()[1];
		...
		for (auto &p : rpGraphs)
		{
			...
			bool sector56 = (p.first.find("R_") == 0);

			p.second.f_x_sh = (sector56) ? new TF1("", "[0]") : new TF1("", "[0] + [1]*x");
			p.second.g_x_sh->Fit(p.second.f_x_sh, "Q");

			p.second.f_y_tilt = new TF1("", "[0]");
