...
xangle = "130";
beta = "0.25";
string ref = "data_alig-version3_fill_6554_xangle_130_beta_0.25_DS1";
*/
...
{
	if (rp == "L_1_F") return GenerateSlices(8.0);
	if (rp == "L_1_N") return GenerateSlices(8.0);
	if (rp == "R_1_N") return GenerateSlices(0.);
	if (rp == "R_1_F") return GenerateSlices(0.);
...
for (int xsi = 0; xsi < n_slices; ++xsi)
{
	...
	for (int rpi : rps.keys)
	{
		...
		RootObject hist = RootGetObject(f, ob + "", error=false);	
		RootObject fit = RootGetObject(f, ob + "|ff_pol1", error=false);	
